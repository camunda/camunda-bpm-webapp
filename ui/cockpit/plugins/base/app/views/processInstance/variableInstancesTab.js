  ...
  ngModule.controller('VariableInstancesController', [
    ...
    function(
      ...
      $scope.headColumns = [
        {
          ...
          sortable: true,
          content: $translate.instant('PLUGIN_VARIABLE_NAME')
        },
        {
          class: 'type',
          request: 'variableType',
          sortable: true,
          content: $translate.instant('PLUGIN_VARIABLE_TYPE')
        },
        {
          class: 'value',
          request: '',
          sortable: false,
          content: $translate.instant('PLUGIN_VARIABLE_VALUE')
        },
        {
          class: 'scope',
          request: '',
          sortable: false,
          content: $translate.instant('PLUGIN_VARIABLE_SCOPE')
        }
      ];
      ...
      function updateView(
        ...
        return variableService.count(countParams).then(function(response) {
          $scope.total = response.count;
          // get variables objects
          return variableService.instances(params).then(function(response) {
            var data = response;
