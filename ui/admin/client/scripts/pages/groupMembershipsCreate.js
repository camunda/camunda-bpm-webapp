...
module.exports = [
  '$scope', '$q', '$location', 'Uri', 'Notifications', 'camAPI', '$modalInstance', 'member', 'memberId', 'idList', 'translateFilter',
  function($scope,   $q,   $location,   Uri,   Notifications,   camAPI,   $modalInstance,   member,   memberId,   idList, translateFilter) {
    ...
    $q.all([ loadAllGroups() ]).then(function(results) {
      ...
      Notifications.addError({
        'status': 'Failed',
        'message': translateFilter('GROUP_MEMBERSHIP_CREATE_LOAD_FAILED') + ': ' + error.message,
        'exclusive': ['type']
      });
    });
    ...
    $scope.createGroupMemberships = function() {
      ...
      deferred.promise.then(function() {
        ...
        Notifications.addError({
          'status': 'Failed',
          'message': translateFilter('GROUP_MEMBERSHIP_CREATE_CREATE_FAILED') + ': ' + error.message,
          'exclusive': ['type']
        });
      });
